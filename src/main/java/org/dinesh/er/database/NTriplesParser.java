...
public class NTriplesParser {
  ...
  private void parseLine() {
    ...
    if (line.charAt(pos) == '<')
      ...
      throw new ErException("Subject in line " + lineno +
    ...
    // property
    if (pos >= line.length())
      throw new ErException("Line ends before predicate on line " + lineno);
    else if (line.charAt(pos) != '<')
      throw new ErException("Predicate does not start with '<', " +
                              "nearby: '" +
    ...
    String object;
    if (pos >= line.length())
      throw new ErException("Line ends before object on line " + lineno);
    else if (line.charAt(pos) == '<')
      object = parseuri();
    else if (line.charAt(pos) == '"') {
      ...
      throw new ErException("Illegal object on line " + lineno + ": " +
                              line.substring(pos));

    // terminator
    skipws();
    if (pos >= line.length() || line.charAt(pos++) != '.')
      throw new ErException("Statement did not end with period; line: '" +
                              line + "', line number: " + lineno);

    skipws();
    if (pos + 1 < line.length())
      throw new ErException("Garbage after period on line " + lineno);
    ...
    for (int ix = 0; ix < literal.length(); ix++)
      if (literal.charAt(ix) == '\\') {
        ...
        if (ch == 'n')
          ...
          throw new ErException("Unknown escaped character: '" + ch + "' in '" + literal + "'");
      } else
  ...
  private String parsebnode() {
    ...
    pos++; // skip '_'
    if (line.charAt(pos++) != ':')
      throw new ErException("Incorrect start of blank node");
