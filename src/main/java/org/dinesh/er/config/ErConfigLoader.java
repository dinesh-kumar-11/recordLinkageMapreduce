...
import org.dinesh.er.cleaners.ChainedCleaner;
...
import org.dinesh.er.datasource.CSVDataSource;
import org.dinesh.er.datasource.Column;
import org.dinesh.er.datasource.ColumnarDataSource;
import org.dinesh.er.datasource.JDBCDataSource;
import org.dinesh.er.datasource.JNDIDataSource;
import org.dinesh.er.datasource.NTriplesDataSource;
import org.dinesh.er.datasource.SparqlDataSource;
import org.dinesh.er.exception.ErException;
import org.dinesh.er.utils.ObjectUtils;
...
/**
 * Can read XML ErConfiguration files and return a fully set up ErConfiguration.
 */
public class ErConfigLoader {

  /**
   * Note that if file starts with 'classpath:' the resource is looked
   * up on the classpath instead.
   */
  public static ErConfiguration load(String file)
    throws IOException, SAXException {
    ErConfigurationImpl cfg = new ErConfigurationImpl();
  ...
  /**
   * Loads the ErConfiguration XML from the given string.
   * @since 1.3
   */
  public static ErConfiguration loadFromString(String config)
    throws IOException, SAXException {
    ErConfigurationImpl cfg = new ErConfigurationImpl();
  ...
  private static class ConfigHandler extends DefaultHandler {
    private ErConfigurationImpl config;
    private List<ErProperty> properties;
    ...
    private ErProperty.Lookup lookup;
    ...
    private ConfigHandler(ErConfigurationImpl config, String path) {
      this.config = config;
      this.properties = new ArrayList<ErProperty>();
    ...
    public void	startElement(String uri, String localName, String qName,
      ...
      if (keepers.contains(localName)) {
        ...
        high = 0.5;
        comparator = null;
        lookup = ErProperty.Lookup.DEFAULT;
        if (attributes.getValue("lookup") != null)
          lookup = (ErProperty.Lookup) ObjectUtils.getEnumConstantByName(
                                ErProperty.Lookup.class,
        ...
        if (!(datasource instanceof ColumnarDataSource))
          throw new ErException("Column inside data source which " +
                                        "does not support it: " + datasource);

        String name = attributes.getValue("name");
        if (name == null)
          throw new ErException("Column with no name");
        String ErProperty = attributes.getValue("property");
        ...
        Column c = new Column(name, ErProperty, prefix, cleaner);
        ...
        if (currentobj == null)
          throw new ErException("Trying to set parameter " +
        ...
        if (groupno == 1 && !config.getDataSources().isEmpty())
          throw new ErException("Cannot have groups in deduplication mode");
        else if (groupno == 3)
          throw new ErException("Record linkage mode only supports " +
                                        "two groups");
    ...
    public void endElement(String uri, String localName, String qName) {
      if (localName.equals("threshold"))
        ...
        if (idprop)
          properties.add(new ErPropertyImpl(name));
        else {
          ErProperty p = new ErPropertyImpl(name, comparator, low, high);
          if (ignore_prop)
      ...
      if (keepers.contains(localName))
        ...
        if (groupno > 0 && groupno != 2)
          throw new ErException("Record linkage mode requires exactly 2 groups; should you be using deduplication mode?");
      }
    }
  ...
  private static Object instantiate(String classname) {
    try {
      ...
      throw new ErException("Couldn't instantiate class " + classname +
                                    ": " + e);
    }
  }
}
