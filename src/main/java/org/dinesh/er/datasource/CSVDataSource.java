...
import org.dinesh.er.utils.CSVReader;
...
public class CSVDataSource extends ColumnarDataSource {
  ...
  public RecordIterator getRecords() {
    ...
    try {
      ...
      throw new ErException("Couldn't find CSV file '" + file + "'");
    } catch (IOException e) {
  ...
  public class CSVRecordIterator extends RecordIterator {
    ...
    public CSVRecordIterator(CSVReader reader) throws IOException {
      ...
      if (hasheader && !getColumns().isEmpty() && header == null)
        throw new ErException("CSV file contained no header");
      ...
      for (Column c : getColumns()) {
        ...
        if (!found)
          throw new ErException("Column " + c.getName() + " not found "+
                                        "in CSV file");
      }
